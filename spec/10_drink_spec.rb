...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:default_ounces) { described_class.new('coffee') }
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(default_ounces.type).to eq('coffee')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(default_ounces.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:default_drink) { described_class.new('_', 15) }

      # remove the 'x' before running this test
      it 'is not full' do
        expect(default_drink).not_to be_full
      end
    end
  end
end
