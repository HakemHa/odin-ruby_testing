...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      ...
      it 'completes loop and displays error message twice' do
        ...
        max = game_input.instance_variable_get(:@maximum)
        error_message = "Input error! Please enter a number between #{min} or #{max}."
        expect(game_input).to receive(:puts).with(error_message).exactly(2)
        game_input.player_input(min, max)
      end
    end
  end
  ...
  describe '#verify_input' do
    ...
    game_verify = described_class.new(1, 10)
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        input = 5
        result = game_verify.verify_input(min, max, input)
  ...
  describe '#maximum_guesses' do
    ...
    context 'when game minimum and maximum is 100 and 600' do
      subject(:game_6_hundred) { described_class.new(100, 600) }
      it 'returns 9' do
        max = game_6_hundred.maximum_guesses
        expect(max).to eq(9)
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    subject(:game_display_turn) { described_class.new(1, 100, random_number) }
    let(:random_number) { instance_double(RandomNumber) }
    let(:binary_search_turn) { instance_double(BinarySearch, make_guess: nil, update_range: nil) }

    before do
      # You'll need to create a few method stubs.
      allow(game_display_turn).to receive(:display_guess).and_return(nil)
    ...
    it 'increases guess_count by one' do
      guess_count = game_display_turn.instance_variable_get(:@guess_count)
      game_display_turn.display_turn_order(binary_search_turn)
      expect(game_display_turn.instance_variable_get(:@guess_count)).to eq(guess_count + 1)
    end
