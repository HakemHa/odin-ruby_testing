...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 0 and max is 9' do
      it 'returns 4' do
        result = (game_guessing.min + game_guessing.max) / 2
        expect(game_guessing.make_guess).to eq(result)
      end
    end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:random_number) { double('random_number') }
      guess = 8
      subject(:found_number) { described_class.new(0, 9, random_number, guess) }
      ...
      it 'is game over' do
        allow(random_number).to receive(:value).and_return(guess)
        expect(found_number).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      ...
      it 'is not game over' do
        allow(random_number).to receive(:value).and_return(guess + 1)
        expect(found_number).not_to be_game_over
      end
    end
  end
