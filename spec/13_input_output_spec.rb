...
describe NumberGame do
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      ...
      it 'completes loop and displays error message twice' do
        expect(game_loop).to receive(:puts).with('Input error!').exactly(2)
        game_loop.player_turn
      end
    end
  end
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      subject(:game_two) { described_class.new(5, '5', rand(2..3)) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        congrats_phrase = "Congratulations! You picked the random number in #{game_two.count} guesses!\n"
    ...
    context 'when count is 4 and over' do
      subject(:game_three) { described_class.new(5, '5', rand(4..10)) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        hard_phrase = "That was hard. It took you #{game_three.count} guesses!\n"
